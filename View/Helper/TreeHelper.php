 ...
 */
	function generate ($data, $settings = array ()) {
		$this->__settings = array_merge(array(
				...
				'itemType' => 'li',
				'id' => false,
				'class' => false,//vou modificar para usar array('depth'=>'nomeDaClasse')
				'element' => false,
				'callback' => false,
				'autoPath' => false,
				'left' => 'lft',
		...
		foreach ($data as $i => $result) {
			...
			if ($__addType) {
				...
				if ($type) {
					...
					//marretei aqui
					if($elementData['depth']>=1){
						$return .= '<' . $type .  $typeAttributes . ' class="dropdown-menu sub-menu">';
					}else{
					//ate aqui
						// echo "entrou aqui".$type.$typeAttributes;
						$return .= '<' . $type .  $typeAttributes . ' class="dropdown-toggle">';
						//$return .='<a href="#" class="dropdown-menu" data-toggle="dropdown">Click Here <span class="caret"></span></a>';
					}
				}
			}
			if ($indent) {
