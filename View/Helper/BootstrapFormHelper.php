...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private function addItemTableCreate($model, $fields)
    {
        ...
        foreach ($this->request->data[$model] as $key => $req_values) {
            ...
            if (empty($req_values['id'])) {
                $html .=
                "<td class='actions'>" .
                $this->Html->link(__("Delete"), "#", array('class' => 'btn btn-danger', 'onclick' => 'cakeUIDeleteRow("' . $model . '-row-' . $key . '","' . $table_id . '"); return false;')) . " " .
                    "</td>";
            } else {
                $html .=
                "<td class='actions'>" .
    ...
    private function addItemJs($model, $div_id, $fields)
    {
        ...
        $this->Html->link(__("Delete"), "#", array('class' => 'btn btn-danger', 'onclick' => '$(this).parents("tr:first").remove();if($("#' . $table_id . ' tbody tr").size()==0){$("#' . $table_id . '").remove();}return false;')) . " " .
            "</td>";
